(function (window, document, $, undefined) {
	...
	$.extend(F, {
		...
		_afterZoomIn: function () {
			...
			if (current.closeBtn) {
				$(current.tpl.closeBtn).prependTo(F.skin).bind( isTouch ? 'touchstart.fb' : 'click.fb', function(e) {
					e.preventDefault();
	...
	F.helpers.title = {
		beforeShow: function (opts) {
			...
			if (!isString(text) || $.trim(text) === '') text = '&nbsp;';

			$('<div class="fancybox-title fancybox-title-inside-wrap">' + text + '</div>').prependTo(F.skin);
		}
	};
	...
	F.helpers.note = {
		beforeShow: function (opts) {
			...
			if ($.isFunction(note)) note = note.call(current.element, current);

			if (!isString(note) || $.trim(note) === '') return;

			$('<div class="fancybox-title fancybox-title-inside-wrap">' + note + '</div>').appendTo(F.skin);;
		}
	};
