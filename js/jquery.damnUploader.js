    ...
    $.fn.damnUploader = function(params, data) {
        ...
        if (!params || $.isPlainObject(params)) {
            ...
            self._damnUploaderUploadItem = function(url, item) {
                ...
                xhr.open("POST", url);
				if (typeof item.data === 'object') item.data = JSON.stringify(item.data);//new
				
                if ($.support.fileSending) {
                    // W3C (Chrome, Safari, Firefox 4+)
                    var formData = new FormData();
					formData.append((item.fieldName || 'file'), item.file);
					formData.append((item.fieldName || 'file'), item.data);//new
                    xhr.send(formData);
        ...
        switch(params) {
            ...
            case 'startUpload':
                ...
                $.each(queue, function(queueId, item) {
					...
					item.data = item.data || set.data; //new
            ...
            case 'setParam':
                var acceptParams = ['url', 'multiple', 'fieldName', 'limit', 'data']; //new
